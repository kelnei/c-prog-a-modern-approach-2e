...
struct part *erase(struct part *erase);
    ...
    for (;;) {
        ...
        switch (code) {
            case 'i': inventory = insert(inventory);
                      break;
            case 'e': inventory = erase(inventory);
                      break;
 ...
 *        removes it from the database if a part with 
 *        that number exists; otherwise, it does nothing.
 */
struct part *erase(struct part *inventory)
{
